...
public:
  ...
  inline constexpr typename mat<4, 3, T>::row_type const &
  operator[](int i) const;

  void operator=(const mat<4, 3, T>&) = delete;
...
template <typename T>
inline constexpr typename mat<4, 3, T>::row_type const &
mat<4, 3, T>::operator[](int i) const
{
  switch (i)
  {
  default: [[fallthrough]];
...
template<typename T>
inline constexpr mat<4, 3, T> operator*(mat<4, 3, T> const& m1, mat<4, 3, T> const& m2)
{
  constexpr T row3[4] = {0, 0, 0, 1};
#define c(i, j) (                               \
    m1[i][0] * m2[0][j]                         \
  + m1[i][1] * m2[1][j]                         \
  + m1[i][2] * m2[2][j]                         \
  ...
  return mat<4, 3, T>(c(0,0), c(0,1), c(0,2), c(0,3),
...
template<typename T>
inline constexpr typename mat<4, 3, T>::row_type operator*
(
  mat<4, 3, T> const& m,
...
#define c(i) (                                  \
  ...
  return typename mat<4, 3, T>::row_type(c(0), c(1), c(2));
#undef c
}

template<typename T>
inline constexpr mat<4, 3, T> transpose(mat<4, 3, T> const& m)
{
  return mat<4, 3, T>(
